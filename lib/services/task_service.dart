...
class TaskService {
  ...
  Future<void> addTask(Task task, String ownerId) async {
    await _firestore.collection('live').doc('tasks').collection(ownerId).add(task.toJsom());
    // _updateTasks(ownerId);

  }

  Future<void> editTask(Task task, String ownerId) async {
    await _firestore.collection('live').doc('tasks').collection(task.ownerId).doc(task.id).update(task.toJsom());
  ...
  void _updateTasks(String ownerId) {
    _firestore.collection('live').doc('tasks').collection(ownerId).snapshots().listen(
        ...
        final List<Task> tasks = snapshot.docs.map(
              (DocumentSnapshot doc) => Task.fromJson(
            doc.id,
            ...
            for (QueryDocumentSnapshot doc in snapshot.docs) {
              if (doc.exists) {
                ...
                if (isShared) {
                  ...
                  print("added: ${sharedTask.toJsom()}... list: ${sharedTasks.toSet().toList()}");
                } else {
